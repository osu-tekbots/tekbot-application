...
$Data = array();
	...
	elseif($_POST['action'] == 'laserUpload') {
		...
		$file_ext=strtolower(end(explode('.',$_FILES['file']['name'])));
		$Data["UploadName"] = $file_name_no_ext.date('Y-m-d-G-i-s').".".$file_ext;
		$extentions= array("dxf", "DXF", "svg", "SVG");
		if(in_array($file_ext,$extentions)=== false){
	...
	elseif($_POST['action'] == 'ticketImageUpload') {
		if(isset($_FILES['file'])){
			$errors= array();
			$file_name = $_FILES['file']['name'];
			$file_size =$_FILES['file']['size'];
			...
			$expensions= array("jpeg","jpg","png","bmp","JPG","JPEG","PNG","BMP","heic","HEIC");
			if(in_array($file_ext,$expensions)=== false){
				$errors[]="extension not allowed, please choose a JPEG or PNG file.";
				$Data["successful"] = 0;
				$Data["string"] = 'extension not allowed, please choose a JPEG or PNG file.';
				echo json_encode($Data);
				exit();
			}
			if($file_size > (5 * 2097152)){
			...
			if(empty($errors)==true){
				$dbfilename = $Data["UploadName"];
				try {
					move_uploaded_file($file_tmp,"../uploads/tickets/".$dbfilename);
					$Data["successful"] = 1;
					$Data["string"] = $dbfilename;
