...
class LaserActionHandler extends ActionHandler {
    ...
    public function handleSendUserEmail() {
        ...
        $ok = $this->mailer->sendEmail($body['email'], 'Laser Cut Submission Follow-up', $body['message'], false, $this->config->getWorkerMaillist());
		if (!$ok) {
            $this->respond(new Response(Response::INTERNAL_SERVER_ERROR, 'Failed to send email to user'));
        }
    ...
    public function handleProcessAllFees() {
        ...
        $ok = $this->mailer->sendToolProcessFeesEmail($unprocessedJobs, 'Cuts', $message, $this->config->getWorkerMaillist());
