...
foreach ($equipment as $e){
	...
	$viewButton = $e->getIsDeleted() ? '' : createLinkButton("pages/publicEquipmentDetailNew.php?id=$equipmentID", 'View');
	$editButton = createLinkButton("pages/employeeEquipmentDetailNew.php?id=$equipmentID", 'Edit');
...
<script>
	$('#createEquipmentBtn').on('click', function () {
		...
		api.post('/equipment.php', data).then(res => {
			window.location.replace('pages/employeeEquipmentDetailNew.php?id=' + res.content.id);
		}).catch(err => {
